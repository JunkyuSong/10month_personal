...
HRESULT CTarget_Manager::Begin_MRT(ID3D11DeviceContext * pContext, const _tchar * pMRTTag, ID3D11DepthStencilView * _pDepthStencil, _float fWidth, _float fHeight)
{
	list<CRenderTarget*>*		pMRTList = Find_MRT(pMRTTag);
	if (nullptr == pMRTList)
		return E_FAIL;
	...
	pContext->OMSetRenderTargets(iNumRTVs, RTVs, _pDepthStencil);
	...
	pContext->RSSetViewports(iNumViewport, &ViewportDesc);
